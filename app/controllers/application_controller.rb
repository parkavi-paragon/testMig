class ApplicationController < ActionController::Base
  ...
  def inventory_token
   ...
   def common_to_fetch_metrics(params,duration)
    collection = []
    date_value = []
    metric_data = []
    duration.each do|date|
	report_result = IncidentLog.chart_report(params[:type],date,params[:frequency],params[:service_center_id]).last
	unless report_result.present?
	        report_result = {}
		report_result[:run_date] = date
		report_result[:metric_value] = 0
	 end
		date_value << rep_date = params[:frequency] == "Monthly" ? report_result[:run_date].strftime("%b") : report_result[:run_date].strftime("%d")
		time_related = time_based_values .include? params[:type]
		metric_data << metric_value = time_related == true ? get_time(report_result[:metric_value]) : report_result[:metric_value]
	end
	collection << date_value
